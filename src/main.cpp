  ...
  if (overlay->header("Settings")) {
    {
      ...
      if (_screenshot_._is_saved && now - _screenshot_._save_time > std::chrono::seconds{5}) {
        _screenshot_._is_saved = false;
      }

      if (overlay->button("Take Screenshot")) {
        _screenshot_._filename = fmt::format("{}.ppm", now.time_since_epoch().count());
        _save_screenshot(_screenshot_._filename.c_str());
      }
      if (_screenshot_._is_saved) {
        const auto text = fmt::format("Screenshot saved as {}", _screenshot_._filename);
        overlay->text(text.c_str());

        _screenshot_._save_time = now;
      }
    }
...
void vulkan_scene_renderer::_save_screenshot(const char* filename) {
  _screenshot_._is_saved = false;
  ...
  format_props = physicalDevice.getFormatProperties(swapChain.colorFormat);
  ...
  format_props = physicalDevice.getFormatProperties(vk::Format::eR8G8B8A8Unorm);
  ...
  vk::Image src_image = swapChain.images[currentBuffer];
  ...
  image_create_ci.extent.width = width;
  image_create_ci.extent.height = height;
  ...
  // Create the image
  vk::UniqueImage dst_image = device.createImageUnique(image_create_ci);
  // Create memory to back up the image
  auto mem_requirements = device.getImageMemoryRequirements(*dst_image);
  auto mem_alloc_info = vks::initializers::memoryAllocateInfo();
  mem_alloc_info.allocationSize = mem_requirements.size;
  // Memory must be host visible to copy from
  mem_alloc_info.memoryTypeIndex = vulkanDevice->getMemoryType(mem_requirements.memoryTypeBits,
                                                               vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
  vk::UniqueDeviceMemory dst_image_memory = device.allocateMemoryUnique(mem_alloc_info);
  device.bindImageMemory(*dst_image, *dst_image_memory, 0);

  // Do the actual blit from the swapchain image to our host visible destination image
  vk::UniqueCommandBuffer copy_cmd = vulkanDevice->createCommandBuffer(vk::CommandBufferLevel::ePrimary, true);
  ...
  vulkanDevice->flushCommandBuffer(copy_cmd, queue);

  // Get layout of the image (including row pitch)
  vk::ImageSubresource subresource{vk::ImageAspectFlagBits::eColor, 0, 0};
  vk::SubresourceLayout subresource_layout = device.getImageSubresourceLayout(*dst_image, subresource);

  // Map image memory so we can start copying from it
  const std::byte* data = static_cast<std::byte*>(device.mapMemory(*dst_image_memory, 0, VK_WHOLE_SIZE, {}));
  ...
  if (!supports_blit) {
    ...
    color_swizzle = (std::find(formats_bgr.begin(), formats_bgr.end(), swapChain.colorFormat) != formats_bgr.end());
  }
  ...
  // Clean up resources
  device.unmapMemory(*dst_image_memory);
  dst_image_memory.reset();
  dst_image.reset();

  _screenshot_._is_saved = true;
}
